/*
[윈도우 배율 설정에 따른 배치깨짐 해결]
프로젝트 속성 -> 매니패스트 도구 -> 입력과 출력 -> DPI 인식 -> 모니터 단위 높은 DPI 인식
...
void DCP::CreateDCP(int a_width, int a_height)
{
	...
	if (mp_graphic == NULL || m_width != a_width || m_height != a_height) 
	{
		...
		// 'AntiAlias' 폰트 사용
		mp_graphic->SetTextRenderingHint(Gdiplus::TextRenderingHintAntiAlias);
	}
}
...
void DCP::SetPenThickness(float a_thickness)
{
	if (a_thickness > 0.0f) 
	{
		mp_pen->SetWidth(a_thickness);
...
void DCP::DCPText(int a_x, int a_y, const TCHAR* ap_str, Gdiplus::ARGB a_text_color)
{
	...
	size_t str_len = _tcslen(ap_str);
	mp_graphic->DrawString(ap_str, static_cast<int>(str_len), mp_font, Gdiplus::PointF(Gdiplus::REAL(a_x), Gdiplus::REAL(a_y)), mp_font_brush);
}

// 텍스트 글꼴과 크기 변경, 글색상
void DCP::DCPTextSetting(float a_font_size, const TCHAR* ap_font_style, Gdiplus::ARGB a_text_color)
{
	float font_size = mp_font->GetSize();
	Gdiplus::FontFamily font_style;
	mp_font->GetFamily(&font_style);
	
	if (ap_font_style != NULL && a_font_size != NULL) {
		delete mp_font;
		mp_font = new Gdiplus::Font(ap_font_style, a_font_size);
	}
	else if (a_font_size != NULL && ap_font_style == NULL) {
		delete mp_font;
		mp_font = new Gdiplus::Font(&font_style, a_font_size);
	}
	else if (a_font_size == NULL && ap_font_style != NULL) {
		delete mp_font;
		mp_font = new Gdiplus::Font(ap_font_style, font_size);
	}
...
void DCP::Polygon(Gdiplus::PointF* ap_position_list, int a_count, Gdiplus::ARGB a_pen_color, Gdiplus::ARGB a_fill_color)
{
	// 채우고
	if (a_fill_color != NULL) {
		m_temp_color.SetValue(a_fill_color);
...
void DCP::FillPolygon(Gdiplus::PointF* ap_position_list, int a_count, Gdiplus::ARGB a_fill_color)
{
	if (a_fill_color != NULL) {
		m_temp_color.SetValue(a_fill_color);
		((Gdiplus::SolidBrush*)mp_brush)->SetColor(m_temp_color);
...
void DCP::DrawPolygon(Gdiplus::PointF* ap_position_list, int a_count, Gdiplus::ARGB a_pen_color)
{
	if (a_pen_color != NULL) {
		m_temp_color.SetValue(a_pen_color);
		mp_pen->SetColor(m_temp_color);
...
// 둥근 사각형 테두리
void DCP::DrawRoundRect(float a_x, float a_y, float a_width, float a_height, float a_radius, Gdiplus::ARGB a_pen_color)
{
	if (a_pen_color != NULL) {
		m_temp_color.SetValue(a_pen_color);
		mp_pen->SetColor(m_temp_color);
	...
	// 각 모서리에 대해 둥근 부분 추가
	path.AddArc(a_x, a_y, a_radius * 2, a_radius * 2, 180, 90);																						// 좌상단 모서리
	path.AddArc(a_x + a_width - (a_radius * 2), a_y, a_radius * 2, a_radius * 2, 270, 90);												// 우상단 모서리
	path.AddArc(a_x + a_width - (a_radius * 2), a_y + a_height - (a_radius*2), a_radius*2, a_radius*2, 0, 90);				// 우하단 모서리
	path.AddArc(a_x, a_y + a_height - (a_radius * 2), a_radius * 2, a_radius * 2, 90, 90);												// 좌하단 모서리
	path.CloseFigure();

	mp_graphic->DrawPath(mp_pen, &path);
}

void DCP::FillRoundRect(float a_x, float a_y, float a_width, float a_height, float a_radius, Gdiplus::ARGB a_fill_color)
{
	if (a_fill_color != NULL) {
		m_temp_color.SetValue(a_fill_color);
		((Gdiplus::SolidBrush*)mp_brush)->SetColor(m_temp_color);
	...
	mp_graphic->FillPath(mp_brush, &path);
}